...
################################
.PHONY: all
all: kubernetes-sa-crb kubernetes-sa-secret kubernetes-harvest vault-policy secrets-enable-kv auth-enable-kubernetes auth-kubernetes-config auth-kubernetes-role #target ## All Targets
...
################################
# K8S: CREATE SERVICEACCOUNT SECRET
################################
.PHONY: kubernetes-sa-secret
kubernetes-sa-secret: #target ## Kubernetes Create ServiceAccount Secret.
...
deploy-app-auth-k8s: #target ## Test Pod Vault Auth Kubernetes
	...
	sleep 5
	kubectl get pods --selector=app=test-auth-k8s
